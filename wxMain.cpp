...
void wxLogoExit(int code) {
}

#define LINEPAUSE 25

// flush the output
void flushFile(FILE * stream) {
...
void wxLogoSleep(unsigned int milli) {
...
void printToScreen(char c, FILE * stream) 
{ 
  if (stream != stdout) {
    putc(c, stream);
    return;
